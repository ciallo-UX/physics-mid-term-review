...
\section*{题目}
\textcolor{blue}{两个匀质圆盘，同轴地粘结在一起，构成一个组合轮。小圆盘的半径为$r=0.100\ \text{m}$，质量为$m=2.00\ \text{kg}$；大圆盘的半径$r'=2r$，质量$m'=2m$。组合轮可绕通过其中心且垂直于盘面的光滑水平固定轴O转动，对O轴的转动惯量$J=\dfrac{9mr^2}{2}$。两圆盘边缘上分别绕有轻质细绳，细绳下端各悬挂质量为$m$的物体A和B。这一系统从静止开始运动，绳与盘无相对滑动，绳的长度不变。则当物体A上升$h$时，组合轮的角速度大小$\omega=[\ ]$。（重力加速度$g=9.80\ \text{m/s}^2$）参数：$h=0.500\ \text{m}$}
...
\subsection*{1. 运动关系}
当物体A上升$h$时，组合轮转过角度$\theta = \dfrac{h}{r}$，物体B下降距离为$2h$。
物体线速度：$v_A=r\omega$，$v_B=2r\omega$。

\subsection*{2. 机械能守恒}
系统初始动能为0，末态动能包括物体动能和轮的转动动能，势能变化为B下降的势能减去A上升的势能：
\[
\frac{1}{2}mv_A^2 + \frac{1}{2}mv_B^2 + \frac{1}{2}J\omega^2 = mg(2h) - mgh
\]
代入$v_A, v_B, J$：
\[
\frac{1}{2}mr^2\omega^2 + 2mr^2\omega^2 + \frac{1}{2} \cdot \frac{9mr^2}{2}\omega^2 = mgh
\]
化简得：
\[
\frac{19}{4}mr^2\omega^2 = mgh \implies \omega^2 = \frac{4gh}{19r^2}
\]

\subsection*{3. 数值计算}
代入$g=9.80, h=0.500, r=0.100$：
\[
\omega = \sqrt{\frac{4 \times 9.80 \times 0.500}{19 \times 0.01}} \approx 10.16\ \text{rad/s}
\]
按两位有效数字取近似，结果为10 rad/s。


\section*{题目}
\textcolor{blue}{体重、身高相同的甲乙两人，分别用双手握住跨过无摩擦轻滑轮的绳子各一端。他们从同一高度由初速为零向上爬，经过一定时间，甲相对绳子的速率是乙相对绳子速率的两倍，则到达顶点的情况是：}
\begin{enumerate}
    \item[A] 甲先到达
    \item[B] 乙先到达
    \item[C] 同时到达
...
\subsection*{1. 系统分析：动量与角动量守恒}
设甲、乙的质量均为$m$，轻滑轮无摩擦、无质量，系统所受合外力矩为零，故系统对滑轮中心轴的角动量守恒。

\subsection*{2. 相对速度与绝对速度关系}
设绳子对地的速率为$u$，取向上为正方向：

- 甲相对绳子的速率为$2v$，则甲对地的速度：$v_{\text{甲}} = 2v - u$

- 乙相对绳子的速率为$v$，则乙对地的速度：$v_{\text{乙}} = v + u$

\subsection*{3. 角动量守恒推导}
初始系统静止，总角动量为0。对滑轮中心轴取角动量，两人的角动量大小相等、方向相反：
\[
m v_{\text{甲}} r = m v_{\text{乙}} r
\]
约去$m$和$r$，得：
\[
v_{\text{甲}} = v_{\text{乙}}
...
即：
\[
2v - u = v + u \implies u = \frac{v}{2}
\]

\subsection*{4. 对地速度计算}
将$u = \frac{v}{2}$代入，得：
\[
v_{\text{甲}} = 2v - \frac{v}{2} = \frac{3v}{2}, \quad v_{\text{乙}} = v + \frac{v}{2} = \frac{3v}{2}
\]
可见两人对地的速度始终相等。

\subsection*{5. 到达时间分析}
两人初始高度相同，对地的速度始终相等，因此到达顶点的时间也相同。


\section*{题目}
\textcolor{blue}{关于力矩有以下几种说法：}

(1) 内力矩不会改变刚体对某个定轴的角动量。

(2) 作用力和反作用力对同一轴的力矩之和必为零。

(3) 质量相等，形状和大小不同的两个刚体，在相同力矩的作用下，它们的角速度一定相等。
...
\frac{dL}{dt} = M_{\text{外}}
\]
其中 \(M_{\text{外}}\) 是外力矩的矢量和。
内力矩是刚体内部各质点间相互作用力产生的力矩，由于作用力与反作用力的力矩相互抵消，**内力矩的矢量和恒为零**，因此内力矩不会改变刚体的角动量。
所以，说法(1)是正确的。

\subsection*{说法(2)分析}
设作用力为 \(\vec{F}\)，作用点位矢为 \(\vec{r}\)；反作用力为 \(-\vec{F}\)，作用点位矢为 \(\vec{r}'\)。
它们对同一轴的力矩分别为：
\[
\vec{M}_1 = \vec{r} \times \vec{F}, \quad \vec{M}_2 = \vec{r}' \times (-\vec{F})
\]
对同一轴取矩，两力矩之和为：
\[
\vec{M}_1 + \vec{M}_2 = (\vec{r} - \vec{r}') \times \vec{F}
\]
由于作用力与反作用力是一对内力，若两力作用于同一点（或在同一直线上），则 \(\vec{r} - \vec{r}' = 0\)，故力矩之和为零。因此，作用力和反作用力对同一轴的力矩之和必为零。
说法(2)是正确的。
...
M = J\beta
\]
其中 \(J\) 为转动惯量，\(\beta\) 为角加速度。
质量相等、形状和大小不同的刚体，其**转动惯量 \(J\) 不同**，在相同力矩 \(M\) 作用下，角加速度 \(\beta = M/J\) 不同。
角加速度不同，经过相同时间后，角速度 \(\omega = \omega_0 + \beta t\) 也不相等。
所以，说法(3)是错误的。

\subsection*{结论}
说法(1)(2)正确，说法(3)错误，故选项A正确。


\section*{题目}
\textcolor{blue}{管内流动时，为克服阻力产生的管路能量损失，若用压强损失$\Delta p$表示时，其物理意义是（）}
\begin{enumerate}
    \item[A] 单位重量流体的能量损失
    \item[B] 单位质量流体的能量损失
    \item[C] 单位体积流体的能量损失
    \item[D] 单位时间经管路流体的能量损失
\end{enumerate}

\section*{解析}
根据流体力学基本概念：
\begin{itemize}
    \item 水头损失$h_f$（单位：m[即J/N]）：表示单位重量【N】流体的能量损失；
    \item 能量损失$w_f$（单位：J/kg）：表示单位质量流体的能量损失；
    \item 压强损失$\Delta p$（单位：Pa = N·$m^{-2}$ = J/m$^3$）：表示单位体积流体的能量损失。
\end{itemize}
因此，用压强损失$\Delta p$表示时，其物理意义是单位体积流体的能量损失


\section*{题目}
\textcolor{blue}{用皮托静压管测量得出的物理量是（ ）}
\begin{enumerate}
    \item[A] 总压与静压的差值
    \item[B] 总压
    \item[C] 静压
...
\section*{解析}
皮托静压管（皮托管）通过迎流开口测得总压，通过侧壁开孔测得静压。它直接输出的物理量是两者的差值，即动压：
\[
p_{\text{动}} = p_{\text{总}} - p_{\text{静}} = \frac{1}{2}\rho v^2
\]
总压、静压、流量均为间接测量或计算量，只有总压与静压的差值是直接测量得出的。

这是它最基础的功能。通过伯努利方程，把测得的总压和静压的差值（动压），换算成流体的实际流速

得到流速后，结合管道截面积就能算出流量：Q=v·A

修正系数C，如果是理想情况，流体无粘【理想流体】、皮托管设计完美，那么 C=1。但实际中总有各种影响因素，所以 C 通常是 0.98∼1.0。


\section*{题目}
\textcolor{blue}{当流场中的流线与迹线重合时，此流动是（ ）}
\begin{enumerate}
    \item[A] 恒定的非均匀流
    \item[B] 恒定均匀流
    \item[C] 恒定流
...
\section*{解析}
流线是某一时刻流场中与速度方向相切的曲线，迹线是流体质点的运动轨迹。
\begin{itemize}
    \item 恒定流（定常流）：流场中各点的速度等参数不随时间变化，此时流线的形状和位置固定，流体质点将沿着流线运动，因此流线与迹线必然重合。
    \item 非恒定流（非定常流）：流场随时间变化，流线不断改变，迹线无法与某一时刻的流线重合。
\end{itemize}
均匀/非均匀是空间分布的概念，与流线和迹线是否重合无关，因此核心条件是恒定流。


\vspace{2em}
\textcolor{blue}{按连续介质的概念，流体质点是指几何尺寸同流体空间相比是极小量，又含有大量分子的微元体}


\section*{题目}
\textcolor{blue}{实际液体在紊流状态，其流层间的切应力为（ ）}
\begin{enumerate}
    \item[A] \(T_1 + T_2\)
    \item[B] 零
    \item[C] 粘性切应力\(T_1\)
...
紊流的切应力由两部分组成：
\begin{enumerate}
    \item 粘性切应力 \(T_1 = \mu \frac{du}{dy}\)，由流体粘性产生；
    \item 附加惯性切应力 \(T_2 = \rho l^2 \left( \frac{du}{dy} \right)^2\)，由质点脉动掺混引起，是紊流特有的。
\end{enumerate}
紊流中两种切应力同时存在，总切应力为两者之和 \(T = T_1 + T_2\)。

\vspace{2em}
\textcolor{blue}{圆管均匀流中，切应力与半径成正比，所以管轴上为零，管壁上达最大值，此规律与流态无关}



\section*{题目}
\textcolor{blue}{静止液体中存在（ ）}
\begin{enumerate}
    \item[A] 压应力
    \item[B] 压应力与拉应力
    \item[C] 压应力与切应力
...
静止液体中：
\begin{itemize}
    \item 切应力由粘性和速度梯度产生，静止时速度梯度为零，因此切应力为零；
    \item 液体几乎无抗拉能力，拉应力不存在；
    \item 液体可承受压力，表现为各向同性的压应力（静压强）。
\end{itemize}
因此，静止液体中仅存在压应力。


\section*{题目}
\textcolor{blue}{雷诺数的物理意义表示（ ）}
\begin{enumerate}
    \item[A] 惯性力与粘滞力之比
    \item[B] 粘滞力与重力之比
    \item[C] 重力与惯性力之比
...
雷诺数的定义式为：
\[
Re = \frac{\text{惯性力}}{\text{粘滞力}} = \frac{\rho v L}{\mu}
\]
它反映了流体流动中惯性力与粘滞力的相对大小，是判断流态（层流/紊流）的重要无量纲数。


\section*{题目}
\textcolor{blue}{欧拉法研究的变化情况（ ）}
\begin{enumerate}
    \item[A] 每个质点的速度
    \item[B] 每个空间的流速
    \item[C] 每个质点的轨迹
...
流体力学研究流动有两种基本方法：
\begin{itemize}
    \item \textbf{拉格朗日法}：以单个流体质点为研究对象，跟踪其运动轨迹、速度等随时间的变化。
    \item \textbf{欧拉法}：以空间中的固定点为研究对象，观测流体流过该点时，流速、压强等物理量随时间的变化情况。
\end{itemize}
因此，欧拉法的核心研究对象是每个空间点的流速变化。

\textcolor{blue}{文丘里流量计就是用粗管和细管，结合连续性+伯努利方程来测量整个管道的流速，进而得到流量；\\皮托管只是测到一点流速，所以只是估算流量}


\section*{题目}
\textcolor{blue}{当管壁的绝对粗糙度 $\Delta$ 大于粘性底层厚度 $\delta_l$ 时，管壁粗糙度对流动能量损失起主要作用，此时管道称为（）}
\begin{enumerate}
    \item[A] 光滑管
    \item[B] 粗糙管
    \item[C] 水力粗糙管
...
紊流流动中，管壁附近存在极薄的粘性底层（层流底层），厚度为 $\delta_l$。
根据绝对粗糙度 $\Delta$ 与 $\delta_l$ 的相对大小，管道分为三类：
\begin{enumerate}
    \item 水力光滑管：$\Delta < \delta_l$，粗糙凸起被粘性底层覆盖，粗糙度对阻力几乎无影响；
    \item 过渡粗糙管：$\Delta \approx \delta_l$，粗糙度与粘性共同影响阻力；
    \item 水力粗糙管：$\Delta > \delta_l$，粗糙凸起伸入紊流区，对能量损失起主要作用。
\end{enumerate}

\vspace{2em}
\textcolor{blue}{如果在同一时刻，流场内各点的物理量与空间坐标无关，则称此流场为（均匀场）}


\section*{题目}
如果在同一时刻，流场内各点的物理量与空间坐标无关，则称此流场为（ ）
\begin{enumerate}
    \item[A] 均匀场
    \item[B] 定常场
    \item[C] 不定常场
...
流体力学中，场的分类有两个独立维度：
\begin{itemize}
    \item \textbf{按空间分布（均匀/不均匀）}：同一时刻，流场内物理量是否随空间坐标变化。
    \item \textbf{按时间变化（定常/不定常）}：同一空间点，物理量是否随时间变化。
\end{itemize}
题目中“同一时刻，物理量与空间坐标无关”，符合均匀场的定义。


\section*{例2 解答}
\textbf{题目：} \textcolor{blue}{小球 $m$ 以 $v_0$ 撞在细木棒 $M$ 末端并粘附，求碰撞后运动及瞬时静止点 $P$。}

\subsection*{(1) 系统运动分析}
\textbf{研究对象：} 棒与球组成的系统。
\textbf{物理过程：} 忽略摩擦，系统动量守恒、对质心角动量守恒。

\subsubsection*{1. 质心位置}
以木棒左端为原点，质心 $C$ 距左端距离：
\[
l_c = \frac{M \cdot (l/2) + m \cdot l}{M + m} = \frac{Ml}{2(M + m)}
\]

\subsubsection*{2. 质心速度 (动量守恒)}
水平方向动量守恒：
\[
mv_0 = (M + m)v_c \implies \boxed{v_c = \frac{mv_0}{M + m}}
\]
\textbf{结论：} 系统质心以 $v_c$ 做匀速直线运动。

\subsubsection*{3. 角速度 (角动量守恒)}
碰撞前对质心的角动量等于碰撞后绕质心的角动量。
系统总转动惯量：
\[
J_{\text{总}} = ml_c^2 + \left( \frac{1}{12}Ml^2 + M\left(\frac{l}{2} - l_c\right)^2 \right)
\]
代入角动量守恒方程解得：
\[
\boxed{\omega = \frac{6mv_0}{(4m + M)l}}
\]
\textbf{结论：} 系统绕质心以 $\omega$ 做匀速转动。

\subsection*{(2) 瞬时静止点 $P$ 的位置}
瞬时静止点满足 $\vec{v}_P = 0$，即绕质心转动速度抵消质心平动速度。
设 $P$ 到质心距离为 $x$，则：
\[
v_c = \omega x
\]
...
x = \frac{(4m + M)l}{6(M + m)}
\]
\textbf{位置：} 点 $P$ 位于质心 $C$ 背离小球一侧，距离质心 $x$ 处。



\section*{例6 解答}
\textcolor{blue}{均匀圆柱体 \(m, R\) 沿倾角 \(\theta\) 的粗糙斜面无滑下滚。}

\subsection*{(1) 质心速度 \(v_c\) 和角速度 \(\omega\)}
纯滚动条件：\(v_c = R\omega\)，摩擦力不做功，机械能守恒：
\[
mg(h_0 - h) = \frac{1}{2}mv_c^2 + \frac{1}{2}J\omega^2
\]
代入 \(J = \frac{1}{2}mR^2\) 和 \(\omega = \frac{v_c}{R}\)：
\[
mg(h_0 - h) = \frac{1}{2}mv_c^2 + \frac{1}{4}mv_c^2 = \frac{3}{4}mv_c^2
\]
解得：
\[
v_c = \sqrt{\frac{4}{3}g(h_0 - h)},\quad \omega = \frac{1}{R}\sqrt{\frac{4}{3}g(h_0 - h)}
\]

\subsection*{(2) 摩擦力条件}
对质心列牛顿第二定律和转动定律：
\[
mg\sin\theta - f = ma_c,\quad fR = J\beta
\]
纯滚动 \(a_c = R\beta\)，联立得：
\[
a_c = \frac{2}{3}g\sin\theta,\quad f = \frac{1}{3}mg\sin\theta
\]
不打滑要求 \(f \le \mu N = \mu mg\cos\theta\)，因此：
\[
\mu \ge \frac{1}{3}\tan\theta
\]
...
\]
    ...
    \item \textcolor{blue}{倾角为 \( \theta \) 的光滑斜面上，一质量为 \( m \) 的滑块与斜面顶端的弹簧相连，弹簧劲度系数为 \( k \)，初始时滑块静止。现给滑块沿斜面向下的初速度 \( v_0 \)，当弹簧伸长量为 \( x \) 时，求滑块的动能大小。}

重力势能变化量应当考虑x-$x_0$,而不是x
    \[
E_k = \frac{1}{2}mv_0^2 + mgx\sin\theta - \frac{1}{2}kx^2 - \frac{(mg\sin\theta)^2}{2k}
\]
\item 电梯内有一质量为 \( m \) 的物体，相对电梯静止。电梯以竖直向下的加速度 \( a = \dfrac{g}{4} \) 向下运动高度 \( h \)，求此过程中电梯对物体做的功。
    ...
    \item \textcolor{blue}{某不稳定粒子静止质量为 \( m_0 \)，固有衰变时间为 \( \tau_0 \)，在实验室参考系中测得衰变时间为 \( \tau \)。求实验室中测得该粒子的动能。}
...
\begin{enumerate}
    ...
    \]
\end{enumerate}
    \item \textcolor{blue}{从上小下大的喷头中竖直向上喷出的水柱最大高度为 \( H \)，喷头出口处高度不计，喷头上截面半径为 \( r \)，下截面半径为 \( R \)，水的密度为 \( \rho \)，大气压强为 \( p_0 \)。求喷头下截面处水的压强。}
\textbf{解：}
\begin{enumerate}
...
\begin{enumerate}
    \item（10分）\textcolor{blue}{光滑水平面上有一长为 \( L \)、质量均匀分布为 \( m \) 的细杆，以垂直于杆方向的平动速度 \( v_0 \) 运动。杆前方有一固定立柱 \( OO' \)，细杆与立柱发生完全非弹性碰撞，碰撞点距杆一端 \( \dfrac{L}{3} \)。求碰撞瞬间细杆的角速度大小。}
\vspace{0.5em}
